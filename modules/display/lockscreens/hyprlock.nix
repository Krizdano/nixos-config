{ packages, lib, config, ...}:
 let
   font = packages.fonts.forHyprlock.name;
 in
   {
     config = lib.mkIf (config.display.lockscreen == "hyprlock") {
       programs.hyprlock = {
         enable = true;
         settings = {
           general = {
           ...
           input-field = {
             ...
             halign = "center";
             valign = "bottom";
             placeholder_text = ''<span font_family="${font}">password</span>'';
             fail_text = ''<span font_family="${font}">Wrong password</span>'';
           };
         };
         extraConfig = ''
           label {
